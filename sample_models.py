...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    rnn = GRU(units, activation="relu", return_sequences=True, implementation=2, name="rnn_1")(input_data)
    bn_rnn = BatchNormalization(name="bnn_1")(rnn)
    if recur_layers == 2:
        rnn = GRU(units, activation="relu", return_sequences=True, implementation=2, name="rnn_2")(bn_rnn)
        bn_rnn = BatchNormalization(name="bnn_2")(rnn)
    # DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
...
def final_model(input_dim=161, units=200, output_dim=29):
    ...
    bidir_rnn1 = Bidirectional(GRU(units, activation='relu', return_sequences=True, implementation=2, name='rnn1'), merge_mode='concat')(input_data)
    bidir_rnn2 = Bidirectional(GRU(units, activation='relu', return_sequences=True, implementation=2, name='rnn2'), merge_mode='concat')(bidir_rnn1)
